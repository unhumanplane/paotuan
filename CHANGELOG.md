...
- 新增 Dashboard 静态兜底入口：插件初始化时会把只读管理页安装到 AstrBot WebUI 同源路径 `/auto-trpg-dm-dashboard/index.html`，插件卡片打不开时仍可直接访问。
- 管理页现在同时支持 AstrBot 插件页 Bridge 和直接访问模式；直接访问模式会读取 WebUI 登录后的 `localStorage.token`，继续通过 `/api/plug/auto_trpg_dm/...` 的 AstrBot JWT 鉴权读取数据。

### Fixed

- 修复管理页 HTML/JS 文案的编码错位，并保留会话快照、审计摘要、备份摘要的只读展示。
...
- 修复开场后未绑定玩家仍可通过“找材料/做饭/分发/继续射击/念咒”等普通场内动作进入 LLM 并写 `update_scene` 的问题；这些动作现在会在本地入口直接要求先建卡/绑定角色。
- 收紧晚加入建卡流程的工具权限：未绑定玩家在角色落库前只能使用建卡、绑定、规则查询和会话控制等安全工具，不能提前写主线场景或推进全团时间线。
