...
| Load strict-grid entities | `load_active_strict_grid_entities(session.maps, session.battle)` | Entity map derived from the same MapStore-first strict-grid precedence, for router auto-advance, turn order/owner checks, and spatial turn guards. | Re-reading `session.battle["grid"].entities` directly in ordinary gameplay readers. |
