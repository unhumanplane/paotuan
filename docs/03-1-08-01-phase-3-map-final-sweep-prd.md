...
- `tools/map_tools.py` still implements explicit legacy `generate_map_svg()` fallback, but new fallback output now writes visual-only MapStore `render_refs` where possible and normalized pending-output metadata; it no longer writes new `scene["last_map_svg"]` active state.
- Fast-path, compact snapshot, prompt relevant-character, and memory summary readers now use MapStore-first strict-grid helpers instead of direct `battle.grid.entities` authority.
- `core/map_core.py` resolves old saves with `battle.map_id` through a MapStore strict record only when `active_strict_map_id` is missing; an existing active strict map still wins over stale `battle.map_id` and `battle.grid`.
...
| 03.1.02 legacy battle-grid adapter | Downgrade `battle.grid` to migration source / compatibility mirror | Spatial tools, battle snapshot, turn/router helpers, renderer fallback, compact battle | `create_grid`, movement, lifecycle, migration, renderer fallback | `session.maps.records[active_strict_map_id].grid` | MapCore helpers, spatial tools, renderer tools, turn/router helpers, compact snapshots, prompt relevant-character selection, memory helpers | MapStore strict grid writers plus compatibility mirror writers | Ordinary runtime readers migrated; mirror writers remain compatibility-only | Turn/router/spatial/main/memory/prompt guards use `load_active_strict_grid*()` helpers | `tests/test_map_core.py`, `tests/test_spatial_tools.py`, `tests/test_turn_tools.py`, `tests/test_router_usage.py`, `tests/test_map_final_sweep.py`, `tests/test_prompts.py` | `docs/mapcore-projection-guard.md`, 03.1.08 PRD | Physical mirror writer surface remains compatibility debt, not authority | No |
| 03.1.03 `create_grid` migration | Make `create_grid` write MapStore first | Existing callers expected `battle.grid` | `create_grid` wrote `battle.grid` as state | `save_active_strict_grid()` plus `battle.map_id` | Spatial tools, strict renderer, turn/router/main/memory helpers | `create_grid` writes MapStore then mirrors `battle.grid` | Mirror kept for compatibility only | Strict renderer and turn/router/main/memory tests read MapStore result | `tests/test_spatial_tools.py`, `tests/test_map_core.py`, `tests/test_map_final_sweep.py` | `docs/mapcore-projection-guard.md` | Mirror remains a physical old-caller output copy | No |
...
| 03.1.06 role projections | Use owner-specific views for DM, RA, renderer, diagnostics | raw snapshots and compact snapshots | snapshot builders | `dm_narration_view`, `ra_authority_view`, `player_view`, `diagnostic_view` | DM prompt, RA authority, renderer tools, diagnostics, compact snapshots | projection helpers only produce filtered views | Raw projected leakage blocked on ordinary paths | Prompt builders, RA builders, compact battle, and renderers use projections / MapStore helpers in runtime | `tests/test_prompts.py`, `tests/test_prompt_projection.py`, `tests/test_environment_agent.py`, `tests/test_map_final_sweep.py` | `docs/projection-consumer-matrix.md`, `docs/mapcore-projection-guard.md` | Full-snapshot debug escape remains diagnostic-only | No |
| 03.1.06.01 projection consumer matrix | Record consumer-specific projection contracts | Consumers could rely on compatibility fields | Docs-only handoff | renderer/player output consumes `player_view + envelope` | strict and overview renderers now consume player-view envelopes | renderer tools write render refs / pending output | Matrix updated for full rollout | Renderer tools are real runtime consumers, not only tests | renderer and prompt projection tests | `docs/projection-consumer-matrix.md` | None for Phase 3; keep matrix current on future renderer changes | No |
| 03.1.07.00 renderer input boundary | Inventory legacy visual entrypoints and define renderer IO | `generate_map_svg`, `last_map_svg`, `_pending_outputs` | legacy map tool wrote SVG, last ref, pending output | `player_view + render envelope` in; render refs and delivery metadata out | strict / overview renderer tools and explicit legacy fallback | render tools write render refs and pending output; legacy fallback writes render refs / normalized pending output only | `last_map_svg` is read-only old-save metadata; legacy path explicit-only | Real renderer tools consume player projections | renderer tool tests, prompt projection tests, final-sweep fallback tests | `docs/coordinate-renderer-contract.md` | None for active `last_map_svg`; keep old-save projection | No |
...
| 03.1.07.02 strict renderer | Deterministic strict-grid SVG over player-safe input | `generate_map_svg` and raw battle grid fallback | legacy SVG path | `render_strict_grid_svg` over `player_view` | renderer tool and delivery path | render refs plus normalized pending output | legacy fallback still available explicit-only | Real tool writes artifact and delivery metadata | `tests/test_strict_grid_render_tools.py`, `tests/test_strict_grid_svg_renderer.py`, `tests/test_strict_grid_render_envelope.py` | `docs/strict-grid-svg-renderer-prd.md` | stale `battle.map_id` / active map mismatch regression covered | No |
| 03.1.07.03 overview renderer | Deterministic overview topology SVG over player-visible topology | `generate_map_svg` or narrative topology assumptions | legacy SVG path | `render_overview_topology_svg` over player-view topology facts | renderer tool and delivery path | render refs, layout cache, pending output | legacy fallback still available explicit-only | Real overview renderer reads player-visible topology facts | `tests/test_overview_topology_renderer.py`, `tests/test_overview_topology_render_tools.py` | `docs/overview-topology-svg-renderer-prd.md` | strict/overview active mismatch negative test covered | No |
| 03.1.07.04 delivery cadence | Deterministic-first routing; `generate_map_svg` explicit-only | normal map requests exposed LLM SVG path | legacy map tool, pending output | `map_tool_routing.py`, deterministic renderer tools, cadence state | registry, prompts, render tools, main pending delivery | renderer tools and explicit legacy fallback | cadence compatible; legacy fallback no longer writes active `last_map_svg` | ordinary map requests expose deterministic renderer tools | `tests/test_map_delivery_cadence.py`, `tests/test_tool_registry.py`, `tests/test_prompts.py`, `tests/test_map_final_sweep.py` | `docs/delivery-cadence-legacy-svg-migration-prd.md` | legacy fallback remains explicit-only compatibility | No |
| 03.1.08 cleanup | Narrow raw battle outputs and ordinary strict-grid readers | raw battle snapshots, raw grid RA payload, stale mirror readers | snapshot tool, tool-result buffers | safe `battle_status` / `tactical_map`; MapStore entity reader | spatial snapshot, RA sanitizer, turn/router/main/memory/prompt helpers | safe snapshot writer; mirror writers remain compatibility-only | ordinary runtime readers migrated | Real runtime helpers read MapStore entities | `tests/test_spatial_tools.py`, `tests/test_cycle_buffer.py`, `tests/test_turn_tools.py`, `tests/test_router_usage.py`, `tests/test_map_final_sweep.py`, `tests/test_prompts.py` | 03.1.08 PRD, `docs/mapcore-projection-guard.md` | physical mirror writer surface remains documented compatibility | No |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `battle.grid` as authority | No ordinary runtime reader should use it when MapStore strict grid exists; only old-save adapter, diagnostics, and compatibility mirror maintenance remain | mirror writers in spatial, strict renderer migration, strict lifecycle | `load_active_strict_grid()` / `load_active_strict_grid_entities()` and projected summaries | Old-save migration and compatibility mirror only | If MapStore has no strict grid, migrate legacy grid into strict record; if `active_strict_map_id` is missing but `battle.map_id` points to a strict MapStore record, resolve that record deterministically | Restore direct reads only as compatibility fallback guarded by MapStore-first precedence | map core, spatial, turn, router, prompts, memory tests | Keep mirror classified compatibility-only; no remaining ordinary authority role |
...
| `scene["last_map_svg"]` | safe old-save projection; legacy prompt snapshot compatibility | old saves only | MapStore `render_refs` and normalized pending output metadata | Read old records only; no active-state meaning | Project only safe fields such as type, title, name, visual-only flag | Restore legacy write if explicit fallback compatibility breaks | prompts and projection tests | New writes removed; keep old-save projection |
| `generate_map_svg` | explicit fallback route, registry, prompt guidance | writes SVG artifact, MapStore render ref when possible, normalized pending output, audit | `render_strict_grid_svg` and `render_overview_topology_svg` | Explicit legacy/fallback/migration only | Old sessions do not require new calls; old pending output handles old artifacts | Restore explicit fallback exposure without restoring ordinary exposure | registry, prompts, projection, delivery, final-sweep fallback tests | Keep only bounded fallback; no normal map route and no new `last_map_svg` state |
...
1. Keep ordinary runtime readers on MapStore-first helpers or safe projections.
2. Keep regression tests for stale mirrors, inconsistent active map IDs, compact snapshots, memory summary paths, and legacy visual fallback output.
3. Keep `battle.grid` writes only as output mirrors after authoritative MapStore writes.
4. Keep active visual metadata on render refs and normalized pending output metadata, not `scene["last_map_svg"]`.
...
- Keeping `generate_map_svg` as an explicit fallback helps migration, but it must remain explicit-only and visual-only; new output now lands in render refs / normalized pending metadata rather than `scene["last_map_svg"]`.
...
| 2 | Route remaining ordinary runtime readers through MapStore | `main.py`, `memory_tools.py`, `core/models.py`, `core/prompts.py`, final-sweep tests | visual fallback cleanup | commit 1 | targeted map/prompt/memory/main tests |
| 3 | Finalize legacy visual metadata boundary | `generate_map_svg`, `last_map_svg`, render refs / pending metadata policy, final-sweep tests | new renderer features | commit 2 | registry, prompts, projection, delivery, renderer tests |
| 4 | Resolve old-save `battle.map_id` compatibility through MapStore | MapCore adapter and strict renderer active/stale map tests | mirror writer removal | commit 3 | map core, strict renderer, spatial, turn/router tests |
...
| 2 | ordinary runtime readers no longer use `battle.grid` authority | focused tests for main/memory/prompts/models/map paths | Yes | clean status after reader migration commit |
| 3 | legacy visual fallback cannot act as active map state | registry/prompt/projection/delivery/renderer tests | Yes | clean status after visual fallback commit |
| 4 | old-save `battle.map_id` resolution is deterministic and does not override active strict maps | map core / strict renderer / spatial / turn-router tests | Yes | clean status after compatibility commit |
