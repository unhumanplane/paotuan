...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `battle.grid` legacy mirror | Strict-grid loader, compatibility/migration code, strict lifecycle mirror sync, old-save tests | `create_grid`, spatial `_save_grid`, strict lifecycle start/resume, legacy grid migration, strict renderer migration side effect | `session.maps.records[active_strict_map_id].grid` plus `battle.map_id` combat link | Keep only as old-save loader or narrow compatibility mirror until all normal readers/writers are moved | If MapStore strict grid is absent, wrap/migrate old `battle.grid` into `strict_local_map.grid`; if MapStore exists, legacy mirror must not override it | Restore mirror reads/writes while keeping MapStore precedence | `test_map_core.py`, `test_spatial_tools.py`, `test_turn_tools.py`, `test_router_usage.py`, `test_strict_lifecycle_tools.py`, `test_strict_grid_render_tools.py` | Normal router/turn/spatial guard readers now use `load_active_strict_grid()` and stale mirror regression tests. Remaining writers are compatibility/migration mirrors for 03.1.08.01 audit, not authority. |
...
| `scene["last_map_svg"]` | prompt scene projection safe-ref path, old save compatibility tests | old scene records only | MapStore `render_refs` and normalized pending output metadata | Read compatibility remains; new outputs should not write this as active state | Safe-project old value as visual metadata only; never use as map facts | Restore safe projection or fallback alias | `test_prompts.py`, `test_prompt_projection.py`, map delivery / final-sweep tests | Downgraded to historical/compat metadata; final sweep removed new `generate_map_svg` writes. |
| `generate_map_svg` tool | explicit fallback route, registry, prompt guidance, tests | writes SVG file, visual-only render ref when possible, normalized `_pending_outputs`, audit | `render_strict_grid_svg` and `render_overview_topology_svg` over `player_view` | Explicit legacy/fallback/style/migration window only; no ordinary map request exposure | Old sessions do not need the tool, but old operator workflows may use fallback during migration | Restore explicit fallback exposure; do not restore ordinary exposure without PRD update | `test_tool_registry.py`, `test_prompts.py`, `test_prompt_projection.py`, `test_map_final_sweep.py` | Finalized as explicit visual-only fallback without new `last_map_svg` state. |
...
- stale `battle.grid` changes turn order, turn ownership, router auto-advance ownership, or spatial turn guards when MapStore strict grid exists;
...
- stale legacy mirror cannot override turn order, turn ownership, router auto-advance labels/ownership, or spatial turn guards;
...
| 2 | Move normal strict-grid authority off legacy mirror | MapStore/legacy grid reader cleanup for router, turn, and spatial turn guards; stale mirror guard tests | prompt/tool schema cleanup, renderer changes, compatibility mirror removal | commit 1 | `tests/test_map_core.py`, `tests/test_spatial_tools.py`, `tests/test_turn_tools.py`, `tests/test_router_usage.py`, `tests/test_strict_lifecycle_tools.py`, `tests/test_strict_grid_render_tools.py` |
...
| 2 | normal strict-grid paths no longer depend on legacy mirror authority | targeted map/spatial/turn/router/lifecycle tests | Yes | clean status after authority cleanup commit |
