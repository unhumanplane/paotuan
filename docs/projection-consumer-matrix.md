...
| Hosted action policy | Code-owned system/DM hosting policy layered on control authority. | Projected hosted status, risk ceiling, safe hosted policy reason, audit ref, timeout/fallback metadata. | Starting hosting from silence/vague absence, raw consent text, private hosting instructions, high-risk action execution without pre-authorization, hidden character/map facts in ordinary prompts. | Non-hosted 120-second timeout remains conservative local auto-action. Explicit `hosted_by_system` records can annotate timeout/auto actions with hosted policy metadata. | Keep risk classification and downgrade in deterministic code; future UX may create/revoke records but must still consume this policy. | `core/hosted_action_policy.py`, `tools/turn_tools.py`, `tests/test_hosted_action_policy.py`, `tests/test_turn_tools.py`, `tests/test_prompts.py`. | Follow-up: 04.3 still owns player transfer/relinquish/reclaim UX and expiration/revoke commands. |
...
7. Keep owner/control authority separate from speaker, actor, and turn actor. Hosted-action policy is code-owned; transfer/reclaim UX remains later 04-stage work, not map projection or prompt prose.
