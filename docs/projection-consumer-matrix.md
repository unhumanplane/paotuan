...
It does not implement a deterministic renderer, remove legacy fields, add hidden dice, or introduce per-character private vision. Hosted-action policy and transfer/reclaim UX now live in the stage-4 control layer and remain consumers of this projection boundary, not replacements for it.
...
| Character/control authority | `dm_narration_view` for ordinary prompts, `ra_authority_view` for maintenance, explicit diagnostic output for admin/debug. | Owner id, active controller id, controller type, public status, risk ceiling, duration/expiry summary, safe reason codes. | Treating speaker, actor, turn actor, stale map focus, or current battle focus as authority; raw consent text; private hosting instructions; diagnostic-only counters or audit payloads in ordinary prompts. | Old saves without `control_authority` remain owner-controlled through `Character.player_id`, `player_character_map`, and MapStore-first entity tags. `control_authority.events` keeps bounded forward-only mutation records. | Keep authority decisions in `core/control_authority.py` and `core/control_transfer.py`; the `control_authority` tool may mutate records only after explicit owner confirmation. Authorization must not move into prompts. | `core/control_authority.py`, `core/control_transfer.py`, `tools/control_tools.py`, `core/models.py`, `tools/spatial_tools.py`, `tools/turn_tools.py`, `tests/test_control_authority.py`, `tests/test_control_transfer.py`, `tests/test_control_tools.py`, `tests/test_prompts.py`, `tests/test_spatial_tools.py`, `tests/test_turn_tools.py`. | 04.4 should audit map/turn/tool paths for stale focus or no-character authority bypass after delegation/reclaim. |
| Hosted action policy | Code-owned system/DM hosting policy layered on control authority. | Projected hosted status, risk ceiling, safe hosted policy reason, audit ref, timeout/fallback metadata. | Starting hosting from silence/vague absence, raw consent text, private hosting instructions, high-risk action execution without pre-authorization, hidden character/map facts in ordinary prompts. | Non-hosted 120-second timeout remains conservative local auto-action. Explicit `hosted_by_system` records can annotate timeout/auto actions with hosted policy metadata. `control_authority(relinquish_to_system)` creates the record shape consumed by hosted policy. | Keep risk classification, downgrade, and system-host recognition in deterministic code; transfer/reclaim UX writes records but does not bypass hosted policy. | `core/hosted_action_policy.py`, `core/control_transfer.py`, `tools/control_tools.py`, `tools/turn_tools.py`, `tests/test_hosted_action_policy.py`, `tests/test_control_tools.py`, `tests/test_turn_tools.py`, `tests/test_prompts.py`. | 04.4 should verify hosted map requests and map-affecting hosted actions do not bypass map visibility, turn rights, resource limits, or irreversible-action policy. |
...
7. Keep owner/control authority separate from speaker, actor, and turn actor. Hosted-action policy and transfer/reclaim UX are code-owned; prompt prose may require confirmation, but authorization and reclaim state changes live in deterministic tools.
