...
| Battle tools | There is no `tools/battle_tools.py`; the battle surface is `tools/spatial_tools.py`, `tools/strict_lifecycle_tools.py`, and `tools/registry.py`. `get_battle_snapshot()` still exposes raw battle/grid output in several tactical toolsets. |
...
- `get_battle_snapshot()` currently returns raw `session.battle` and a `grid.to_dict()` payload. Treat it as a follow-up risk, not a safe player-view or ordinary narration contract.
...
| Battle tools | Internal deterministic grid access; exposed gameplay summaries should be projected or sanitized. | Strict grid for local calculation, turn order, action economy, combat lifecycle, map link state, safe tactical summaries. | Raw `battle.grid`, raw `grid.to_dict()`, hidden entities, backend geometry, stale legacy mirror as authority, raw battle dump to ordinary DM/player context. | `get_battle_snapshot()` currently returns raw battle/grid and is exposed in several ordinary tactical toolsets. `battle.grid` mirror remains for old callers and saved sessions. There is no separate `tools/battle_tools.py`; battle-facing tools are split across spatial, strict lifecycle, turn, and registry code. | Add a safe battle/status projection or restrict raw battle snapshot to diagnostic/authority paths in a later PR. | `tools/spatial_tools.py`, `tools/strict_lifecycle_tools.py`, `tools/turn_tools.py`, `tools/registry.py`, `tests/test_spatial_tools.py`, `tests/test_strict_lifecycle_tools.py`, `tests/test_map_core.py`. | Main residual risk: `get_battle_snapshot()` is not diagnostic-only. Treat it as follow-up before broad renderer/player delivery. |
...
2. Replace or restrict `get_battle_snapshot()` before broad player-facing map delivery. Its current raw battle/grid shape is not a safe ordinary narration or player contract.
